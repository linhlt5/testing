...
danh_sach_cau_hoi = danh_sach_cau_hoi ? danh_sach_cau_hoi : [];
    ...
    if(noi_dung_cau_hoi.value === ""){
        ...
        if(Kiem_tra_ans()&& Kiem_tra_checked()){
            if(confirm("Bạn chắc chắn thêm câu hỏi chứ? ")){
                var questionObject = luu_cau_hoi();
...
var editIndex = -1;
function sua(index){
    if(danh_sach_cau_hoi[index].Trangthai === "Đã duyệt" || danh_sach_cau_hoi[index].Trangthai === "Không được duyệt" ){
        alert("Câu hỏi đã được phản ánh, bạn không thể sửa câu hỏi");
    }else{
        if(confirm("Bạn muốn sửa câu hỏi ?")){
            editIndex = index;
        
            var question = danh_sach_cau_hoi[index];
    ...
    if(Kiem_tra_ans()&& Kiem_tra_checked()){
        if(editIndex !== -1){
           var questionObject = luu_cau_hoi();
            ...
            danh_sach_cau_hoi[editIndex] = questionObject;
            localStorage.setItem('Câu hỏi',JSON.stringify(danh_sach_cau_hoi)) ;
            renderQuestion();
            clear();
            hideALL();
